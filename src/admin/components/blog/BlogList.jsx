...
const BlogList = ({ data, isLoading, refetch, next, prev, page, count }) => {
  ...
  const columns = [
    columnHelper.accessor((row) => row.title, {
      id: "Title",
      cell: (info) => <div className="w-[180px] whitespace-normal">{info.getValue()}</div>,
      header: (info) => info.column.id,
    }),
    columnHelper.accessor((row) => row.brief, {
      id: "Brief",
      cell: (info) => (
        <p className="whitespace-normal !w-[300px]">{info.getValue()}</p>
      ),
      header: (info) => info.column.id,
    }),
